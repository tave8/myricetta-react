...
export default class Helper {
  ...
  static normalizeString(x) {
    if (!this.isString(x)) {
      throw new StringIsNotStringError(`During normalizing input to string, 
                                        param "${x}" is expected to be string, 
  ...
  static normalizeNumber(x) {
    ...
    if (!this.isNumber(y)) {
      throw new NumberIsNotNumberError(`During normalizing input to number, 
                                      param "${x}" is expected to be number, 
                                       it is of type "${typeof x}" instead.`)
    }
    return y
  }
  ...
  static isEmptyString(x) {
    if (!this.isString(x)) {
      throw new StringIsNotStringError(`String expected, but param "${x}" 
                                      is of type "${typeof x}" instead.`)
    }
    return x.trim() == ""
  }
  ...
  static roundNumber(num, nDigits) {
    if (!this.isNumber(num)) {
      throw new NumberIsNotNumberError(`Param "number" with value "${num}" 
                                          must be a number and it is not. 
                                          It is of type "${typeof num}" instead.`)
    }
    if (!this.isNumber(nDigits)) {
      throw new NumberIsNotNumberError(
        `Param "number of digits" with value "${num}" 
        must be a number and it is not. 
        It is of type "${typeof nDigits}" instead.`,
      )
    }
